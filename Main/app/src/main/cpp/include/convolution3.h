//
// Created by 카카오VX on 11/7/21.
//

#ifndef MAIN_CONVOLUTION3_H
#define MAIN_CONVOLUTION3_H
...
void Convolution55(IplImage* src[], IplImage* dst, float kernel[32][5][5], float bias)
{
    ...
    for (int row = 0; row < dst->height; row++)
    {
        ...
        {
            double temp = 0;	//�洢dst[row][col]����ֵ16

            for (int i = 0; i < CONV2_FILTERS; i++)
            {
                double temppixel = 0;	//src2[i]��row-col�¾���ֵ
                for (int m = 0; m < 5; m++)
                {
                    for (int n = 0; n < 5; n++)
                    {
                        unsigned int t = 0;	//ȡ��src2[i].[row+m][col+n]����ֵ
